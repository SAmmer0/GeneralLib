...
from .utils import Stock, Portfolio, EqlWeightCalc, MonRebCalcu, MkvWeightCalc, stock_filter_template
...
def bt_test():
    ...
    def mkv_filter(date, st_provider=st_provider, fmkv_provider=fmkv_provider,
        ...
        return out
    quote_provider = HDFDataProvider(
        r'E:\factordata\basicfactors\quote\CLOSE.h5', start_time, end_time)
...
def bt_test2():
    ...
    st_provider = HDFDataProvider(factor_dict['ST_TAG']['abs_path'],
                                  start_time, end_time)
    fmkv_provider = HDFDataProvider(factor_dict['TOTAL_MKTVALUE']['abs_path'],
                                    start_time, end_time)
    tradestatus_provider = HDFDataProvider(factor_dict['TRADEABLE']['abs_path'],
                                           start_time, end_time)
    ...
    def mkv_filter(date, st_provider=st_provider, fmkv_provider=fmkv_provider,
        ...
        return out
    quote_provider = HDFDataProvider(
        r'F:\factordata\basicfactors\quote\ADJ_CLOSE.h5', start_time, end_time)
...
def bt_test3():
    ...
    start_time = '2013-01-01'
    end_time = '2017-08-20'
    st_provider = HDFDataProvider(factor_dict['ST_TAG']['abs_path'], start_time, end_time)
    tradestatus_provider = HDFDataProvider(
        factor_dict['TRADEABLE']['abs_path'], start_time, end_time)
    quote_provider = HDFDataProvider(factor_dict['ADJ_CLOSE']['abs_path'], start_time, end_time)
    tmkv_provider = HDFDataProvider(factor_dict['TOTAL_MKTVALUE']['abs_path'], start_time, end_time)
    tmkv_stock_filter = stock_filter_template(st_provider, tradestatus_provider, 10)
    conf = BacktestConfig(start_time, end_time, quote_provider, EqlWeightCalc(), tradestatus_provider,
                          MonRebCalcu(start_time, end_time))
    bt = Backtest(conf, tmkv_stock_filter, fd_provider=tmkv_provider)
    bt.run_bt()
    return bt
